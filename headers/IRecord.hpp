...
#define IRecord_OVERRIDE                                                       \
    std::string getTable() const noexcept override;                            \
...
class IRecord {
private:
    virtual std::string getTable() const noexcept = 0;
...
public:
    ...
    static std::vector<RecordType>
    select(const std::vector<Condition> &conditions = {});
};

template <class RecordType>
std::vector<RecordType>
IRecord::select(const std::vector<Condition> &conditions) {
    static_assert(
        std::is_base_of<IRecord, RecordType>::value,
    ...
    SQLite::run(
        ...
        [](void *retPtr, int columnCount, char **columnValues, char **names) {
            ((std::vector<RecordType> *) retPtr)
